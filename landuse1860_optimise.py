...
valid_generator = data_generator(images_valid, labels_valid, n_files=4, batch_size=128, target_class=None)
test_generator  = data_generator(images_test,  labels_test,  n_files=4, batch_size=128, target_class=None)
...
# Initialises model
model = unet_model(input_shape=(256, 256, 3), n_outputs=len(classes), filters=64, output_activation='softmax', dropout=0.1, montecarlo=True)
model.summary()
...
# pd.to_pickle(training, f"{paths['models']}/{model.name}.pkl")

#%% ESTIMATES SPECIFIC MODELS

# Defines target class
for target in ['buildings', 'transports', 'crops', 'meadows', 'pastures', 'specialised', 'forests', 'water']:
    ...
    # Initialises model
    model = unet_model(input_shape=(256, 256, 3), n_outputs=1, filters=64, output_activation='sigmoid', dropout=0.1, montecarlo=True)
    model.summary()
    ...
    # Transfers parameters from target model
    if np.any(index):
        model.set_weights(params.get_weights())
    # Transfers parameters from base model
    else:
...
def final_model(input_shape:dict=(256, 256, 3), n_outputs:int=len(classes)):
    # Loads models
    ...
    probas  = layers.concatenate(probas, axis=3, name='concatenate')
    outputs = layers.Conv2D(filters=n_outputs, kernel_size=(3, 3), padding='same', activation='softmax', name='output')(probas)
    model   = models.Model(inputs=inputs, outputs=outputs, name='final_model')
    return model

#%% EVALUATES MODELS

# Loads model
model  = search_data(paths['models'], pattern=f'{target}\.h5$')
...
for image, label, proba, predict in zip(images[index], labels[index], probas[index], predicts[index]):
    ...
    axs[3].imshow(predict, cmap=cmap, norm=norm)
